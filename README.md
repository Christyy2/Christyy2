# 👨‍💻Christy Gérys LAMBERT
<img width="300" height="300" alt="profil" src="https://github.com/user-attachments/assets/76abafcb-7b91-4ca7-87e2-e94ec91bb985" />

---
![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=30&pause=1000&color=FFFDD0&width=800&height=60&lines=Bienvenue+sur+mon+profil+!;Je+suis+développeuse+passionnée)

---

## 📋Table des matières
- [À propos](#à-propos)
- [Compétences](#-compétences)
- [Projets](#-projets)
- [Expériences](#-expériences)
- [Contact](#contact)

---

## À propos
Je suis **Christy Gérys LAMBERT**, mes amis m'appellent m'appellent *Tracy*, je suis une développeuse Web Full Stack, étudiante en 3e année à l'**Institut Universitaire des Sciences** basé à Jacmel,HT.
> *« Le meilleur code est celui qui n'existe pas encore —
> celui que vous allez écrire pour changer quelque chose. »* 

## 🛠 Compétences
1. Frontend
- HTML5
- CSS3
- JS
  
2. Backend
- PHP
- React native
- Python
  
3. Administration Systèmes
- Linux
- Windows

4. Réseaux
- Cisco Packet Tracer
- Wireshark

5. Outils & Environnememt
- Git / Github
- VS Code

6. En apprentissage
- [ ] Kali Linux
- [ ] React Native
- [ ] Machine Learning
- [x] Modélisation Mathématique  

## 🚀 Projets
[Mon portfolio](https://christyy2.github.io/christy-portfolio.github/)

## 💼 Expériences
| Entreprise | Poste | Période | Type |
|:---------:|:---------:|:---------:|:--------:|
| **ChrisTech & Co**| Développeuse Web Full Stack | 2022 - Présent | CDI |
| **Digi-C.A.R** | Secrétaire Générale et Monitrice | Janvier 2026 - Présent | CDD |
| **Freelance** | Pentester | 2025 – Présent | Indépendant |

<div align="center">
  <h2>Disponible pour des projets freelance</h2>
  <p>Contactez-moi et construisons quelque chose ensemble.</p>
</div>

```mermaid
flowchart LR
    A[📋 Analyse\ndu besoin] --> B[🎨 Maquette]
    B --> C[💻 Développement]
    C --> D[🧪 Tests]
    D --> E[🚀 Mise en ligne]
    D -->|Bug trouvé| C
```
<p align="center">
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript" />
</p>

## Contact
- 📧 Email : chrisbert2003@gmail.com
- 🐙 GitHub : https://github.com/Christyy2


---
<p align="center">
  From Christy Gérys LAMBERT@2026
</p>
<!---
Christyy2/Christyy2 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
